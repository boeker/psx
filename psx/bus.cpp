...
Bus::Bus()
    : cdrom(this), cpu(this), dma(this), executable(this), interrupts(this), gpu(this), timers(this) {
    reset();
}
...
template <typename T>
T Bus::debugRead(uint32_t address) {
    T value = 0;
...
template uint32_t Bus::debugRead<uint32_t>(uint32_t address);
template uint16_t Bus::debugRead<uint16_t>(uint32_t address);
template uint8_t Bus::debugRead<uint8_t>(uint32_t address);
